    ...
    >
      <div className="mx-auto flex h-20 w-full max-w-6xl items-center justify-between px-5 sm:px-8">
        ...
        <nav className="hidden items-center gap-8 lg:flex" aria-label="Primary">
          {navigation.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-sm font-medium text-night-900/80 transition-colors hover:text-gold-600"
            >
              {item.label}
            </a>
          ))}
        ...
        <button
          ...
          className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-night-900/15 bg-cream-50/80 text-night-900 lg:hidden"
      ...
      {open ? (
        <div className="fixed inset-0 z-50 flex flex-col bg-night-950 lg:hidden">
          <div className="flex h-20 items-center justify-between px-5 sm:px-8">
            <Logo dark />
            <button
              type="button"
              onClick={() => setOpen(false)}
              className="inline-flex h-11 w-11 items-center justify-center rounded-full border border-cream-50/20 text-cream-50"
